...
=== Changing a workout to be current : `current`

Changes a workout to be a current workout in the workout book. +
Format: `current INDEX`

****
* Changes a workout tag from future or completed to current at the specified `INDEX`. The index refers to the index number shown in the displayed workout list. The index *must be a positive integer* 1, 2, 3, ...
****

Examples:

* `current 1` +
Changes the `future` or `completed` tag in the 1st workout to be `current`.

=== Changing a workout to be completed : `complete`

Changes a workout to be a completed workout in the workout book. +
Format: `current INDEX`

****
* Changes a workout tag from future or current to completed at the specified `INDEX`. The index refers to the index number shown in the displayed workout list. The index *must be a positive integer* 1, 2, 3, ...
****

Examples:

* `complete 1` +
Changes the `future` or `current` tag in the 1st workout to be `completed`.
